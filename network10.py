...
# IP地址分成四份，每份转化为二进制地址，与子网掩码进行逻辑与运算
	...
	# 计算子网掩码 和各个子网起始，终结IP
    print(mask_str)
    print("hostip="+str(hostip)+",""maskdec="+str(maskdec)) 
    netip = hostip & maskdec # 计算
    #netip_str = str(netip)  # 转为字符串
    #maskadd = netip_str[0:mask_count-1]
    #hostadd = netip_str[mask_count:]
    print("netip="+str(netip))
    ips = '{0:0>8}'.format(netip)
    maskip = ips[:mask_count]
    hostips =ips[mask_count:]
    print(ips)
    print(maskip,hostips)
